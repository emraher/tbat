...
data_financial <- function(periods,
  ...
  request_body <- paste0('{"route":"degerler","donemler":[', periods_body,
  ...
  dat <- httr::POST("https://verisistemi.tbb.org.tr/api/router",
    ...
    dplyr::select(.data$banka_tr_adi, .data$yil, .data$ay, .data$tp_deger,
                  .data$yp_deger, .data$toplam, tidyselect::everything())

  if (is.null(banks) & !is.null(groups)) {
    dat <- dat %>% dplyr::left_join(groups_financial(),
      ...
      dplyr::left_join(tables_financial(bank_code = groups),
                       by = "unique_key") %>%
      dplyr::select(.data$banka_tr_adi, .data$banka_eng_adi, .data$yil,
    ...
    dat <- dat %>%
      ...
      dplyr::select(.data$banka_tr_adi, .data$yil, .data$ay, .data$tr_adi,
